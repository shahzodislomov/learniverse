...
interface ChallengeModalProps {
  challenge: IChallenge & { isSolved?: boolean } | null
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}
...
export default function ChallengeModal({
  ...
  const fetchSolvers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/challenges/${challenge._id}/solvers`)
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/challenges/${challenge._id}/submit`, {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        ...
        if (data.firstBlood) {
          toast.success(`🩸 First Blood! You earned ${data.points} points!`)
        } else {
          toast.success(`Correct! You earned ${data.points} points!`)
        }
        // Refresh solvers list
        setTimeout(() => {
          fetchSolvers()
        }, 500)
        setTimeout(() => {
              ...
              <div className="p-6">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <motion.input
                        ...
                        placeholder="CTFVexura{...}"
                        className="w-full px-4 py-3 bg-hacker-dark border border-hacker-green/30 rounded focus:outline-none focus:border-hacker-green focus:shadow-glow-green-sm text-hacker-green font-mono"
                ...
                {!success && (
                  <div className="mt-6">
                    ...
                    {isLoadingSolvers ? (
                      <div className="text-center py-4 text-gray-400">Loading solvers...</div>
                    ) : solvers.length === 0 ? (
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              ...
                              <Link
                                ...
                                className="text-hacker-green hover:text-hacker-cyan transition-colors font-semibold"
                              >
                                {solver.username}
                              </Link>
                            </div>
